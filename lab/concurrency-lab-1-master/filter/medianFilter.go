...
func filter(filepathIn, filepathOut string, threads int) {
	...
	immutableData := makeImmutableMatrix(getPixelData(img)) //closure
	var newPixelData [][]uint8

	if threads == 1 {
		newPixelData = medianFilter(0, height, 0, width, immutableData)
	} else {
		workerHeight := height / threads     //需要运行的高度等于高度除以线程数
		s := make([]chan [][]uint8, threads) //channel
		for i := range s {
			s[i] = make(chan [][]uint8) //二维中的一维传参
		}

		for i := 0; i < threads; i++ {
			go worker(i*workerHeight, (i+1)*workerHeight, 0, 512, immutableData, s[i])

		}
		for i := 0; i < threads; i++ {
			part := <-s[i]                               //从 s[i] 接收数据，并且传给part
...
func worker(startY, endY, startX, endX int, data func(y, x int) uint8, out chan<- [][]uint8) {
	medianFilter := medianFilter(startY, endY, startX, endX, data) //一定要等于
	out <- medianFilter
}
